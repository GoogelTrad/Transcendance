...
function Tournament() {
  ...
  useEffect(() => {
       if (tournamentStarted && !socketRef.current) {
           const newSocket =  new WebSocket(`${process.env.REACT_APP_SOCKET_IP}ws/tournament/${tournamentCode}/?token=${token}`);
       ...
       //   }
       // };
     }, [tournamentStarted, tournamentCode, token, user, fetchTournementCode, navigate]);
